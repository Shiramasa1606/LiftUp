...
@app.route('/proyectos/<int:id>', methods=['GET', 'PUT'])
@jwt_required()
def manejar_proyecto(id):
    conn = connect_db()
    cursor = conn.cursor()
    ...
    if request.method == 'GET':
        try:
            query = """
            SELECT nombre, descripcion, montoObjetivo, fechaLimite, idCategoria
            FROM proyectos WHERE idProyecto = ?
            """
            ...
            return jsonify({
                ...
                'objetivoFondos': proyecto[2],
                'fechaTermino': proyecto[3],
                'categoria': proyecto[4],
            }), 200
        except Exception as e:
            return jsonify({'error': str(e)}), 500
        finally:
    ...
    if request.method == 'PUT':
        try:
            ...
            query = """
            UPDATE proyectos
            SET nombre = ?, descripcion = ?, montoObjetivo = ?, fechaLimite = ?, idCategoria = ?
            WHERE idProyecto = ?
            """
            cursor.execute(query, (
                data['titulo'], data['descripcion'], data['objetivoFondos'], 
                data['fechaTermino'], data['categoria'], id
            ))
            conn.commit()
